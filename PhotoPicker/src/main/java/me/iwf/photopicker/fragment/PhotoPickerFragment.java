...
public class PhotoPickerFragment extends Fragment {
    ...
    private final static String EXTRA_PREVIEW_MODE = "extra_preview_mode";
    private final static String EXTRA_PREVIEW_PHOTOS = "extra_preview_photo";
     ...
     * @return
     */
    public static PhotoPickerFragment newPhotoPreviewInstance(boolean isTakePhoto, boolean previewEnable, ArrayList<String> photoPaths) {
        Bundle args = new Bundle();
        args.putBoolean(EXTRA_CAMERA, false);
        args.putBoolean(EXTRA_GIF, false);
        args.putBoolean(EXTRA_PREVIEW_ENABLED, previewEnable);
        args.putBoolean(EXTRA_TAKE_PHOTO, isTakePhoto);
        args.putInt(EXTRA_COLUMN, PhotoPicker.DEFAULT_COLUMN_NUMBER);
        args.putInt(EXTRA_COUNT, PhotoPicker.DEFAULT_MAX_COUNT);
        args.putStringArrayList(EXTRA_PREVIEW_PHOTOS, photoPaths);
        args.putBoolean(EXTRA_PREVIEW_MODE, true);
        PhotoPickerFragment fragment = new PhotoPickerFragment();
        fragment.setArguments(args);
        return fragment;
    }

    public static PhotoPickerFragment newTakePhotoInstance() {
        Bundle args = new Bundle();
        args.putBoolean(EXTRA_CAMERA, false);
        args.putBoolean(EXTRA_GIF, false);
        args.putBoolean(EXTRA_TAKE_PHOTO, true);
        args.putInt(EXTRA_COLUMN, PhotoPicker.DEFAULT_COLUMN_NUMBER);
        args.putInt(EXTRA_COUNT, PhotoPicker.DEFAULT_MAX_COUNT);
        args.putBoolean(EXTRA_PREVIEW_MODE, true);
        PhotoPickerFragment fragment = new PhotoPickerFragment();
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        isTakePhoto = getArguments().getBoolean(EXTRA_TAKE_PHOTO);
        if (isTakePhoto) {
            openCamera();
        } else {
            boolean isPreviewMode = getArguments().getBoolean(EXTRA_PREVIEW_MODE);
            if (isPreviewMode) {
                ArrayList<String> photoPaths = getArguments().getStringArrayList(EXTRA_PREVIEW_PHOTOS);
                PhotoDirectory directory = new PhotoDirectory();
                directory.setId("preview");
                directory.setCoverPath(photoPaths.get(0));
                List<Photo> temp = new ArrayList<>();

                for (int i = 0; i < photoPaths.size(); i++) {
                    temp.add(new Photo(i, photoPaths.get(i)));
                }
                directory.setPhotos(temp);
        ...
        if (requestCode == ImageCaptureManager.REQUEST_TAKE_PHOTO && resultCode == RESULT_OK) {
            ...
            if (isTakePhoto) {
                ...
                if (!TextUtils.isEmpty(path)) {
                    photos.add(path);
                    ((PhotoPickerActivity)getActivity()).returnResult(photos);
                }
            } else {
                captureManager.galleryAddPic();
